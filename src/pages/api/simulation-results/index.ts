...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getSimulationResults() {
    const data = await prisma.simulation_result
      .withAuthorization({
        roqUserId,
        tenantId: user.tenantId,
        roles: user.roles,
      })
      .findMany(convertQueryToPrismaUtil(req.query, 'simulation_result'));
    return res.status(200).json(data);
  }

  async function createSimulationResult() {
    await simulationResultValidationSchema.validate(req.body);
    const body = { ...req.body };

    const data = await prisma.simulation_result.create({
      data: body,
    });
    return res.status(200).json(data);
  }
