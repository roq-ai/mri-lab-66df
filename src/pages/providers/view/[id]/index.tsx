...
import { deleteSimulationById } from 'apiSdk/simulations';
...
function ProviderViewPage() {
  ...
  const simulationHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('simulation', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/simulations/create?provider_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.simulation?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/simulations/view/${record.id}`)}
                            key={record.id}
                          >
                            ...
                            <Td>
                              {hasAccess('simulation', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/simulations/edit/${record.id}`}
                                >
                              ...
                              {hasAccess('simulation', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    simulationHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
